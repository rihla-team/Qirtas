    ...
    def deactivate_extension(self, ext_id):
        ...
        try:
            if ext_id in self.active_extensions:
                ...
                # 6. إزالة الموديول من sys.modules لض��ان إعادة تحميله بشكل نظيف
        ...
        for ext_id, extension in self.active_extensions.items():
            if hasattr(extension, 'get_context_menu_items'):
                try:
                    ...
                    if items:
                        # إضافة معرف الإضافة لكل عنصر قائمة
                        for item in items:
                            item['extension_id'] = ext_id
                        menu_items.extend(items)
                except Exception as e:
                    self.log_message(f"خطأ في الحصول على عناصر القائمة من الإضافة {ext_id}: {str(e)}", "ERROR")
