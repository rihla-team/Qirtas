...
class ArabicTerminal(QTextEdit):
    ...
    def execute_command(self, command):
        """تنفيذ الأمر"""
        if command.strip():
            # إضافة الأمر الجديد مع توقيت دقيق يشمل الميلي ثانية
            command_entry = {
                'command': command,
                'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            }
            
            # التحقق من عدم تكرار نفس الأمر بنفس التوقيت
            if self.command_history:
                ...
                if isinstance(last_entry, dict):
                    if (last_entry['command'] == command and 
                        ...
                        # تجاهل الأمر إذا كان مكرراً في نفس الثانية
                        return
        ...
        # معالجة خاصة لأمر تغيير المجلد
        if cmd in ["cd", "انتقل", "اذهب"]:
            path = " ".join(parts[1:]) if len(parts) > 1 else ""
            self.change_directory(path)
            self.display_prompt()
            return
        if cmd in self.tools:
            self.tools[cmd].execute(parts[1:])
            self.display_prompt()
            return
        
        # التحقق من أمر السجل بشكل خاص
        if cmd in ['سجل', 'تاريخ', 'تاريخ_الاوامر']:
            self.show_history()
        # التحقق من أمر التشغيل
        if cmd in ["run", "شغل", "تشغيل", "نفذ"]:
            if len(parts) < 2:
                self.append_text("\nخطأ: يجب تحديد اسم الملف للتشغيل\n", self.colors['error'])
                self.display_prompt()
                return
            ...
            try:
                self.append_text(f"\nجاري تشغيل: {file_path}\n", self.colors['output'])
                
                # تشغيل البرنامج في نافذة منفصلة
                if platform.system() == "Windows":
                    # استخدام start لفتح نافذة جديدة
                    subprocess.Popen(f'start cmd /K "{file_path}"', shell=True)
                else:
                    # في Linux نستخدم xterm أو terminal مناسب
                    subprocess.Popen([file_path])
                
                # لا نضيف موجه الأوامر هنا لأن البرنامج يعمل في نافذة منفصلة
                return
        ...
        # التحقق مما إذا كان الأمر عربياً
        is_arabic = any('\u0600' <= c <= '\u06FF' for c in cmd)
        ...
        for command_key, info in self.commands.items():
            if (cmd == command_key or 
                (cmd in info.get('arabic', []))):
                found_command = command_key
                # حفظ الأمر العربي إذا كان الإدخال عربياً
                if is_arabic and cmd in info.get('arabic', []):
                    arabic_command = cmd
                elif is_arabic:
                    arabic_command = info.get('arabic', [''])[0]
                break
        
        if found_command:
            # استخدام الأمر العربي إذا كان متوفراً
            display_command = arabic_command if arabic_command else found_command
            
            # عرض الأمر كما كتبه المستخدم
            self.append_text(f"\n{display_command}", self.colors['input'])
            
            # معالجة خاصة لأمر تغيير المجلد
            if cmd in ["cd", "انتقل", "ذهب"]:
                # تجميع المسار كاملاً (في حالة وجود مسافات)
                path = " ".join(parts[1:]) if len(parts) > 1 else ""
                ...
                try:
                    # تغيير المجلد
                    os.chdir(path)
                    # عرض المسار الجديد بتنسيق أفضل
                    current_path = os.getcwd()
                    formatted_path = self.format_path(current_path)
                    self.append_text(f"\nتم الانتقال إلى: {formatted_path}", self.colors['output'])
                except Exception as e:
                    self.append_text(f"\nخطأ: لا يمكن الانتقال إلى المجلد - {str(e)}", self.colors['error'])
                    
                self.display_prompt()
                return
            
            # التحقق من أمر المسح
            if cmd in ["clear", "cls", "مسح", "تنظيف"]:
                self.clear_terminal()
                return
            
            # التحقق من أمر المساعدة
            if cmd in ["مساعدة", "اوامر", "أوامر", "تعليمات", "الاوامر"]:
                self.display_arabic_help()
                return
            
            # التحقق من أمر الطباعة
            if cmd in ["echo", "اطبع", "اكتب", "صدى"]:
                text = " ".join(parts[1:])
                ...
                self.append_text(f"\n{text}", self.colors['output'])
                self.append_text("\n", self.colors['output'])
                self.display_prompt()
                return
            # التحقق من أمر عرض محتويات المجلد
            if cmd in ["ls", "dir", "عرض", "قائمة"]:
                path = " ".join(parts[1:]) if len(parts) > 1 else "."
                self.list_directory_arabic(path)
                return
            try:
                # إعادة إنشاء العملية
                self.setup_process()
                ...
                cmd = self.commands[found_command][os_type]
                command = f"{cmd} {' '.join(parts[1:])}"
                
                if platform.system() == "Windows":
                    if command.lower().startswith("dir") or command.lower().startswith("ls"):
                        full_command = f"powershell.exe -Command Get-ChildItem"
                    else:
    ...
    def _display_directory_info(self, directory, items_info, total_files, total_folders, total_size):
        """عرض معلومات المجلد - تم ف��لها لتحسين الأداء"""
        # عرض المسار
...
class TerminalTabWidget(QTabWidget):
    ...
    def get_current_terminal(self):
        """الحصول على موج�� الأوامر  الحالي"""
        return self.currentWidget()

    def closeEvent(self, event):
        """حفظ التاريخ ع��د إغلاق البرنامج"""
        try:
            self.save_history()
