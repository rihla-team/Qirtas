...
كل ملحق يجب أن يحتوي على فئة رئيسية تمتد من `BaseExtension`:

```python
from utils.extensions_manager import BaseExtension

class MyExtension(BaseExtension):
    def init(self, editor):
        super().init(editor)
        self.name = "اسم الملحق"

    def initialize(self):
        """يتم استدعاء هذه الدالة عند تفعيل الملحق"""
        pass

    def cleanup(self):
        """يتم استدعاء هذه الدالة عند إيقاف الملحق"""
        pass
```
